...
/*
 * IMPORTANT:
 * Replace the four challenge parameter objects below with the exact
 * parameters from your previous Pixabay API Practice Lab.
 *
 * The current values are simple challenge defaults so the project can
 * be tested immediately. Do not submit until they match your previous lab.
 */
const CHALLENGES = {
    rocketLaunch: {
        label: "Rocket Launch",
...
function displayVideos(hits) {
    ...
    hits.forEach(video => {
        ...
        source.src = video.videos.medium?.url || video.videos.small?.url;
...
function runChallenge(challengeKey) {
    ...
    const endpoint = challenge.endpoint === "videos"
        ? "https://pixabay.com/api/videos/"
        : "https://pixabay.com/api/";
